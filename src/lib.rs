...
fn load_assets(assets_path: &str, animation_data: &Value) -> Result<HashMap<String, Asset>, BodymovinError> {
    let mut assets = HashMap::new();
    if let Some(assets_array) = animation_data["assets"].as_array() {
        for asset in assets_array {
            let id = asset["id"].as_str().ok_or_else(|| BodymovinError::OtherError("Missing asset id".to_string()))?;
            let path = format!("{}/{}", assets_path, asset["p"].as_str().ok_or_else(|| BodymovinError::OtherError("Missing asset path".to_string()))?);
...
pub fn save_frame(frame: &ImageBuffer<Rgba<u8>, Vec<u8>>, output_dir: &str, frame_number: u32) -> Result<(), BodymovinError> {
...
pub fn get_all_frames(bodymovin_json: &str, assets_dir: &str) -> Result<Vec<ImageBuffer<Rgba<u8>, Vec<u8>>>, BodymovinError> {
    ...
    let frames: Result<Vec<ImageBuffer<Rgba<u8>, Vec<u8>>>, BodymovinError> = (0..total_frames).into_par_iter()
        .map(|frame_number| {
